...
def replay_sensor(fname, window_size=1024, frame_size=8192, shift=128):
    ...
    while i_start + window_size - 1 < len_x - 1:
        ...
        print(nd)
        Phs = Phs - Phs[int(frame_size/8)]  # ** added 2021.1.26 kotaro**
        ...
        #1 peak_count[i_frame] = len(signal.argrelmax(Cep_H[0:int(quef)],order=3)[0]) 

        #tameshi
        PowCep_H = np.real(Cep_H * np.conj(Cep_H))
        peak_count[i_frame] = len(signal.argrelmax(PowCep_H[0:int(quef)],order=5)[0])


        #plt(peak_count[i_frame])        
        '''
        for i in peak_value
        peak_valuel = peak_valuel + peak_value
        number = i++
        '''
